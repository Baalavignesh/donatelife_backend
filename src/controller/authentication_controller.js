...
let RegisterUser = async (req, res) => {
  try {
    ...
    console.log(
      `No existing user found with username '${userData.username}', proceeding with registration`
    );
...
let RegisterDonor = async (req, res) => {
  try {
    ...
    if (existingUser) {
      console.log(
        `User with username '${orgData.username}' already exists in the database`
      );
      return res.status(400).json({
        success: false,
        message: "User with that username already exists",
    ...
    console.log(
      `No existing user found with username '${orgData.username}', proceeding with registration`
    );
